export const cnOpt = {
  tooltip: {
    ...
    formatter: function(e) {
      return `${e.seriesName} </br> 累计确诊: ${e.value} </br> 现存确诊:${e.data.econNum}</br> 死亡:${e.data.deathNum} </br>治愈:${e.data.cureNum}`
    },
    show: true,
...
export const worldOpt = {
  tooltip: {
    //   triggerOn: 'click',
    formatter: function(e) {
      return `${e.seriesName} </br> 累计确诊: ${e.data.conNum} </br> 现存确诊:${e.data.econNum}</br> 死亡:${e.data.deathNum} </br>治愈:${e.data.cureNum}`
    },
    show: true,
    transitionDuration: 0, //echart防止tooltip的抖动
  },
